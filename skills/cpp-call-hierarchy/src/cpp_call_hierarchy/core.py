...
def inspect_call_hierarchy_by_symbol(
    query: str,
    ...
    *,
    ...
    with ClangdClient(config) as client:
        ...
        return _inspect(client, symbol.location.path, symbol.location.range.start, config, query, symbol, incoming, outgoing, limit, timeout)


def inspect_call_hierarchy_at(
    ...
    *,
    ...
    with ClangdClient(config) as client:
        return _inspect(client, path, position, config, f"{path}:{line}:{column}", None, incoming, outgoing, limit, timeout)
...
def _inspect(
    ...
    items = client.prepare_call_hierarchy(path, position, timeout=timeout)
    if not items:
        raise ToolError("clangd did not return call hierarchy items")
    item = items[0]
    payload: dict[str, Any] = {
        "query": query,
        "symbol": symbol_to_dict(symbol, config.project_root) if symbol else None,
        "item": call_item_to_dict(item, config.project_root),
        "incoming": [],
        "outgoing": [],
    }
    if incoming:
        payload["incoming"] = [
            incoming_to_dict(call, config.project_root) for call in client.incoming_calls(item, timeout=timeout)[:limit]
        ]
    if outgoing:
        try:
            payload["outgoing"] = [
                outgoing_to_dict(call, config.project_root) for call in client.outgoing_calls(item, timeout=timeout)[:limit]
            ]
            payload["outgoing_resolution"] = "clangd"
        except ToolError:
            payload["outgoing"] = static_outgoing_calls(item, config.project_root, limit=limit)
            payload["outgoing_resolution"] = "fallback-static"
    return payload
...
def call_item_to_dict(item: dict[str, Any], project_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "kind": int(item.get("kind") or 0),
        "kind_name": SYMBOL_KINDS.get(int(item.get("kind") or 0), f"Kind{int(item.get('kind') or 0)}"),
...
def locate_symbol_in_source(query: str, project_root: Path) -> SymbolCandidate | None:
    ...
    for path in candidate_source_files(project_root, leaf):
        ...
        for pattern in symbol_patterns(names):
            ...
            return SymbolCandidate(leaf, container, 6, location, (4, len(query), 0))
    return None
...
def static_outgoing_calls(item: dict[str, Any], project_root: Path, *, limit: int) -> list[dict[str, Any]]:
    ...
    for match in re.finditer(r"\b(?P<name>[A-Za-z_]\w*(?:::\w+)?|operator\s*[^\s(]+)\s*\(", masked_body):
...
def symbol_patterns(names: list[str]) -> list[re.Pattern[str]]:
    ...
    return [re.compile(rf"\b(?P<name>{leaf})\s*\(")]
